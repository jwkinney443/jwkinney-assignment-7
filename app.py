...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = beta0 + beta1 * X + mu + errors # Replace with code to generate Y
    ...
    for _ in range(S):
        ...
        errors = np.random.normal(0, np.sqrt(sigma2), N)
        Y_sim = beta0 + beta1 * X_sim + mu + errors  # Replace with code to generate simulated Y values
...
def hypothesis_test():
  ...
  # Two-tailed test
    ...
    plt.axvline(observed_stat, color='r', linestyle='--', label="Observed Statistic")
...
def confidence_interval():
    ...
    plt.errorbar(mean_estimate, 0, xerr=[[mean_estimate - ci_lower], [ci_upper - mean_estimate]], fmt='o', 
                 color="blue" if includes_true else "red", label="Mean Estimate + Confidence Interval")
